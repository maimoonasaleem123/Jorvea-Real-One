...
const ProfessionalReelItem: React.FC<ReelItemProps> = ({
  ...
  const viewTrackedRef = useRef(false); // Prevent multiple view counts
  ...
  // Track view when reel becomes active (separate effect to prevent loops)
  useEffect(() => {
  ...
  }, [isActive, hasViewed, reel.id]); // Add reel.id to dependencies
  ...
  }, [liked, onLike, reel.id, heartAnimation, actionButtonsScale]);
...
const ProfessionalReelsScreen: React.FC = () => {
  ...
  const loadReels = useCallback(async () => {
    try {
      ...
      // Extract reels array from the response object
  ...
  const handleComment = useCallback((reel: Reel) => {
    // For now, show an alert since Comments screen is not in navigation
    // TODO: Implement proper comments modal or navigate to comments screen
    console.log('Opening comments for reel:', reel.id);
  }, []);

  // Handle scroll to track current reel (throttled to prevent rapid updates)
