...
const FastSearchScreen: React.FC = () => {
  ...
  const loadExploreContent = useCallback(async (refresh = false) => {
    ...
    try {
      ...
      const [posts, reels, stories] = await Promise.all([
        FirebaseService.getExplorePosts(30), // More content like Instagram
        FirebaseService.getExploreReels(30),
        FirebaseService.getActiveStories(), // Add stories for discovery
      ]);
  ...
  const loadTrendingData = useCallback(async () => {
    try {
      const trending = await FirebaseService.getTrendingHashtags();
      setTrendingHashtags(trending.slice(0, 10));
      
      // Load user's recent searches from storage
      const recent = await FirebaseService.getUserRecentSearches(user?.uid || '');
  ...
  const renderExploreItem = useCallback(({ item, index }: { item: ContentItem; index: number }) => {
    ...
    return (
      ...
      >
        ...
        {!isReel && (data as Post).mediaUrls?.length > 1 && (
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {activeTab === 'explore' ? (
        <FlatList
          ...
          getItemLayout={(data, index) => ({
            length: ITEM_SIZE,
            offset: ITEM_SIZE * Math.floor(index / GRID_SIZE),
            index,
          })}
        />
      ) : (
        <FlatList
          data={searchResults}
          renderItem={renderUserItem}
          keyExtractor={(item) => (item as User).uid}
          showsVerticalScrollIndicator={false}
          refreshControl={
            <RefreshControl
              refreshing={refreshing}
      ...
      <InstagramPostViewer
        ...
        posts={selectedContent.map(item => item.data as Post)}
      ...
      <InstagramReelViewer
        ...
        reels={selectedContent.map(item => item.data as Reel)}
        initialIndex={selectedIndex}
